...
impl CPU
{
	...
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.load_memory_u16(mem, seg, addr.wrapping_add((dis as i16) as u16))
	}

	pub fn get_ireg_u16d_u16_value(&self, mem: &Memory, ir: &IndReg, dis: u16) -> u16
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.load_memory_u16(mem, seg, addr.wrapping_add(dis))
	}
	...
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.load_memory_u8(mem, seg, addr.wrapping_add((dis as i16) as u16))
	}

	pub fn get_ireg_u16d_u8_value(&self, mem: &Memory, ir: &IndReg, dis: u16) -> u8
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.load_memory_u8(mem, seg, addr.wrapping_add(dis))
	}
	...
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.store_memory_u8(mem, seg, addr.wrapping_add((dis as i16) as u16), val)
	}

	pub fn set_ireg_u16d_u8_value(&self, mem: &mut Memory, ir: &IndReg, dis: u16, val: u8)
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.store_memory_u8(mem, seg, addr.wrapping_add(dis), val)
	}
	...
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.store_memory_u16(mem, seg, addr.wrapping_add((dis as i16) as u16), val)
	}

	pub fn set_ireg_u16d_u16_value(&self, mem: &mut Memory, ir: &IndReg, dis: u16, val: u16)
	{
		let (seg, addr) = self.get_ireg_addr(ir);
		self.store_memory_u16(mem, seg, addr.wrapping_add(dis), val)
	}
}
